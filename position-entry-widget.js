...
class PositionManageWidget extends HTMLElement {
  ...
  getSelectedIds() {
    ...
    for (i = 0; i < this._rows.length; i++) {
      if (this._rows[i].selected === true && this._rows[i].employeeId) {
        ids.push(this._rows[i].employeeId);
      }
    }
  ...
  setCellValue(rowIndex, fieldName, value) {
    ...
    var readonlyEl = this.shadowRoot.querySelector('.readonly-cell[data-row="' + rowIndex + '"][data-field="' + fieldName + '"]');
    if (readonlyEl) {
  ...
  _validateAllRows() {
    ...
    for (i = 0; i < this._rows.length; i++) {
      ...
      if (r.employeeId && employeeMap[r.employeeId] > 1) {
        rowErr.push("Duplicate Employee ID in table");
      }
  ...
  _createDropdownPanel() {
    ...
    var panel = document.createElement("div");
    panel.className = "position-manage-dropdown-panel";
    panel.style.display = "none";
  ...
  _render() {
    ...
    var html = `
      <div class="wrap">
        ...
        <div class="summary">
          ...
          <div>Validation: ${this._validationResult}</div>
          <div>Error Rows: ${this._validationErrors.length}</div>
          <div>Modified Rows: ${this._getModifiedCount()}</div>
        </div>
      </div>
    `;
    ...
    this.shadowRoot.getElementById("btnSave").addEventListener("click", function() {
      that.triggerSave();
    });
  ...
  _renderCell(row, rowIndex, col, rowErrors) {
    ...
    if (col.type === "readonly") {
      return `
        <div class="readonly-cell" data-row="${rowIndex}" data-field="${col.key}">${this._escapeHtml(String(value))}</div>
        ${this._renderFieldErrors(col.key, rowErrors)}
      `;
    }
  ...
  _renderFieldErrors(fieldName, rowErrors) {
    ...
    for (var i = 0; i < rowErrors.length; i++) {
      var msg = rowErrors[i];
      if (
        (fieldName === "employeeId" && msg.indexOf("Employee ID") === 0) ||
        (fieldName === "companyCode" && msg.indexOf("Company Code") === 0) ||
        (fieldName === "division" && msg.indexOf("Division") === 0) ||
        (fieldName === "department" && msg.indexOf("Department") === 0) ||
        (fieldName === "costCenter" && msg.indexOf("Cost Center") === 0) ||
...
if (!customElements.get("com-example-position-manage")) {
  customElements.define("com-example-position-manage", PositionManageWidget);
}

(function() {
  if (document.getElementById("position-manage-dropdown-global-style")) {
    return;
  }

  var style = document.createElement("style");
  style.id = "position-manage-dropdown-global-style";
  style.textContent = `
    .position-manage-dropdown-panel {
      background: #ffffff;
      border: 1px solid #cfd9e3;
      border-radius: 8px;
      box-shadow: 0 8px 24px rgba(34,53,72,0.18);
    ...
    .position-manage-dropdown-panel .dropdown-search-wrap {
      padding: 8px;
      border-bottom: 1px solid #e8eef5;
      background: #ffffff;
    }
    .position-manage-dropdown-panel .dropdown-search-input {
      width: 100%;
      height: 32px;
      border: 1px solid #b9cae0;
      border-radius: 6px;
    ...
    .position-manage-dropdown-panel .dropdown-search-input:focus {
      border-color: #0a6ed1;
      box-shadow: 0 0 0 2px rgba(10,110,209,0.12);
    }
    .position-manage-dropdown-panel .dropdown-list {
      max-height: 280px;
      overflow: auto;
      background: #ffffff;
    }
    .position-manage-dropdown-panel .dropdown-item {
      padding: 9px 10px;
      font-size: 13px;
      color: #223548;
      cursor: pointer;
    ...
    .position-manage-dropdown-panel .dropdown-item:hover {
      background: #edf5ff;
    }
    .position-manage-dropdown-panel .dropdown-item.selected {
      background: #e8f2ff;
      color: #0a6ed1;
      font-weight: 600;
    }
    .position-manage-dropdown-panel .dropdown-empty {
      padding: 12px;
      color: #7b8a9a;
      text-align: center;
      font-size: 13px;
