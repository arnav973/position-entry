...
class PositionEntryWidget extends HTMLElement {
  ...
  setCellValue(rowIndex, fieldName, value) {
    ...
    if (trigger) {
      ...
      if (labelEl) {
        labelEl.textContent = this._getOptionText(rowIndex, fieldName, value) || "Select";
      }
    }
  }
  ...
  _render() {
    ...
    var style =
      '<style>' +
        ...
        '.gridWrap { overflow:auto; max-height:460px; background:#ffffff; }' +
    ...
    var c = 0;
    for (c = 0; c < this._columns.length; c++) {
      html += '<th style="width:' + this._columns[c].width + '">' + this._columns[c].label + '</th>';
    }
    ...
    for (r = 0; r < this._rows.length; r++) {
      ...
      for (c = 0; c < this._columns.length; c++) {
        html += '<td style="width:' + this._columns[c].width + '">' + this._renderCell(row, r, this._columns[c], rowErrors) + '</td>';
      }
    ...
    html += '<div>Error Rows: ' + this._validationErrors.length + '</div>';
    html += '</div>';
    html += '</div>';
  ...
  _renderCell(row, rowIndex, col, rowErrors) {
    ...
    if (col.type === "checkbox") {
      return '<input class="cell ' + errClass + '" data-row="' + rowIndex + '" data-field="' + col.key + '" data-type="' + col.type + '" type="checkbox" ' + (value === true ? "checked" : "") + ' />';
    }
    ...
    if (col.type === "select") {
      ...
      return ''
        + '<div class="dropdown-trigger ' + errClass + '" tabindex="0" data-row="' + rowIndex + '" data-field="' + col.key + '" data-type="' + col.type + '">'
        +   '<span class="label">' + this._escapeHtml(String(selectedText)) + '</span>'
        +   '<span class="arrow">▼</span>'
        + '</div>'
        + this._renderFieldErrors(col.key, rowErrors);
