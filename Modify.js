class PositionManageWidget extends HTMLElement {
  ...
  setOptions(fieldName, optionsStr) {
    ...
    // no render — call commitRowOptions() after all setOptions calls
  }
  ...
  setRowOptions(rowIndex, fieldName, optionsStr) {
    try {
      ...
      // resolve visible position index → actual row by employeeId
      var visibleCount = 0;
      var i = 0;
      for (i = 0; i < this._rows.length; i++) {
    ...
    // no render — call commitRowOptions() after all setRowOptions calls
  }
  ...
  _getOptionsForCell(rowIndex, fieldName) {
    // look up by employeeId — safe against index shifting
    ...
    for (i = 0; i < this._columns.length; i++) {
      if (this._columns[i].key === "selected") {
        html += '<th style="width:' + this._columns[i].width + '">'
          ...
          + '<input class="select-all-checkbox" type="checkbox" id="selectAll" '
          + (allSelected ? "checked" : "") + ' title="Select Current Page" /></div></th>';
      } else {
  ...
  _renderCell(rowData, rowIndex, columnData, rowErrors) {
    ...
    if (columnData.type === "select") {
      ...
      if (!displayText) { displayText = "Select"; }
